...
import { 
  Camera, ShieldCheck, Upload, CheckCircle, 
  AlertTriangle, Package, Shield, BarChart3, 
  Users, Globe, Clock, Heart, Award, 
  Phone, Mail, MapPin, Facebook, 
  Twitter, Linkedin, Instagram, Youtube,
  ChevronRight, ArrowRight, Star, Truck,
  Headphones, Shield as ShieldIcon, Leaf,
  Zap, Lock, FileCheck, Download
} from "lucide-react";
...
const features = [
  {
    ...
    description: "Every product is registered on an immutable blockchain ledger",
    color: "bg-blue-100 text-blue-600"
  },
  {
    icon: Zap,
    title: "Real-time Tracking",
    description: "Track your medicine from manufacturer to pharmacy in real-time",
...
const howItWorks = [
  {
    ...
    description: "Use our scanner or enter the unique code from your medicine packaging",
    icon: Camera
  },
  {
    step: "02",
    title: "Instant Verification",
    description: "Our system checks against the official database in milliseconds",
    icon: Zap
  },
  {
    step: "03",
    title: "Get Results",
    description: "Receive detailed verification report with manufacturing details",
    icon: CheckCircle
  }
];
  ...
  const handleVerifyCode = useCallback(
    async (codeToVerify: string) => {
      ...
      try {
        ...
        const response = await fetch(`/api/verify_drug?assetId=${encodeURIComponent(trimmedCode)}`, {
        ...
        if (data.isValid) {
          ...
          router.push(`/verified?error=not-found&batchId=${encodeURIComponent(trimmedCode)}`);
        }
      } catch (error) {
        console.error("Verification Error:", error);
        router.push(`/verified?error=network&batchId=${encodeURIComponent(trimmedCode)}`);
      } finally {
        if (timeoutId) {
  ...
  return (
    <div className="relative">
      ...
      <section className="relative min-h-screen flex items-center justify-center px-4 py-8 sm:py-12 md:py-16 overflow-hidden">
        ...
        {/* Gradient Overlay */}
        <div className="absolute inset-0 bg-gradient-to-br from-blue-900/90 via-blue-800/80 to-emerald-900/70" style={{ zIndex: -1 }} />
        ...
        <div className="relative max-w-6xl w-full mx-auto grid md:grid-cols-2 gap-8 md:gap-12 items-center">
          ...
          >
            <div className="inline-flex items-center gap-2 bg-white/10 backdrop-blur-sm px-4 py-2 rounded-full border border-white/20">
              ...
              <span className="text-white font-medium">Trusted By Health Care Professionals </span>
            </div>
            ...
            <p className="text-xl text-white/90 leading-relaxed">
              Every year, millions suffer from counterfeit medications. 
              We provide a secure verification system to ensure every medicine 
              you take is <span className="font-semibold text-emerald-300">authentic, safe, and traceable</span>.
            </p>
            ...
            <div className="pt-4">
              ...
              <AnimatePresence>
                {showStory && (
                  ...
                  >
                    <h3 className="text-xl font-semibold text-white mb-3">The Verification Story</h3>
                    <ul className="space-y-3 text-white/90">
                      <li className="flex items-start gap-3">
                        <CheckCircle className="w-5 h-5 text-emerald-400 mt-1 flex-shrink-0" />
                        <span>Track every medicine from manufacturer to consumer</span>
                      </li>
                      <li className="flex items-start gap-3">
                        <AlertTriangle className="w-5 h-5 text-amber-400 mt-1 flex-shrink-0" />
                        <span>Detect counterfeit drugs before they reach patients</span>
                      </li>
                      <li className="flex items-start gap-3">
                        <Shield className="w-5 h-5 text-blue-400 mt-1 flex-shrink-0" />
                        <span>Ensure compliance with global pharmaceutical standards</span>
                      </li>
                    </ul>
                  </motion.div>
                )}
          ...
          >
            <div className="text-center mb-8">
              ...
              <p className="text-gray-600 mt-2">Scan, upload, or enter your medicine code</p>
            </div>
            ...
            <div className="space-y-4">
              ...
              <label className={`w-full flex items-center justify-center gap-3 bg-gradient-to-r from-emerald-600 to-emerald-700 text-white py-4 px-6 rounded-xl transition-all duration-300 cursor-pointer ${uploadingCodeImage || verifying
